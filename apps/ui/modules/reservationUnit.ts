...
import { isPriceZero } from "common/src/helpers";
...
export function getFuturePricing(
  ...
  const part = pricings
    .filter((p) => new Date(p.begins) > new Date())
...
export function isReservationUnitPaid(
  ...
  return d.filter((p) => !isPriceZero(p)).length > 0;
}
...
export function isReservationUnitPaidInFuture(
  ...
  return (
    pricings.filter((p) => isFuturePricing(p)).filter((p) => !isPriceZero(p))
      .length > 0
  );
}
